...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
    ...
    @Query("SELECT COUNT(i) FROM Invoice i WHERE i.deliveryMethod = invoice_insight_api.enums.DeliveryMethod.DIGITAL")
    long countDigitalInvoices();

    @Query("SELECT COUNT(i) FROM Invoice i WHERE i.status = invoice_insight_api.enums.InvoiceStatus.PAID AND i.paymentDate IS NOT NULL")
    long countPaidWithPaymentDate();

    @Query("SELECT COUNT(i) FROM Invoice i WHERE i.status = invoice_insight_api.enums.InvoiceStatus.PAID " +
            "AND i.paymentDate IS NOT NULL AND i.paymentDate <= i.dueDate")
    long countPaidOnTime();

    @Query("SELECT i.invoiceYear, i.invoiceMonth, COUNT(i) FROM Invoice i " +
            "WHERE i.status = invoice_insight_api.enums.InvoiceStatus.PAID AND i.paymentDate > i.dueDate " +
            "GROUP BY i.invoiceYear, i.invoiceMonth ORDER BY i.invoiceYear, i.invoiceMonth")
