...
public interface CustomerRepository extends JpaRepository<Customers, Long> {
    ...
    @Query("SELECT c.city, COUNT(c) FROM Customers c WHERE c.status <> invoice_insight_api.enums.Status.DELETED " +
            "GROUP BY c.city ORDER BY COUNT(c) DESC")
    List<Object[]> countGroupedByCity(Pageable pageable);

    @Query("SELECT c.gender, COUNT(c) FROM Customers c WHERE c.status <> invoice_insight_api.enums.Status.DELETED " +
            "GROUP BY c.gender")
    List<Object[]> countGroupedByGender();

    @Query("SELECT c.birthDate FROM Customers c WHERE c.status <> invoice_insight_api.enums.Status.DELETED")
