...
import { getSubscriptions } from "../../features/organization/organizationService";
import { formatDate } from "../../features/organization/format";
...
export default function SubscriptionListPage() {
  const { data: subscriptions, loading, error } = useAsyncData(getSubscriptions, []);
  ...
  const filtered = useMemo(() => {
    ...
    return (subscriptions ?? []).filter((subscription) => {
      const matchesSearch =
        normalized === "" ||
        subscription.phoneNumber.includes(normalized) ||
        subscription.employeeName.toLowerCase().includes(normalized) ||
        subscription.subscriptionNumber.toLowerCase().includes(normalized);

      const matchesStatus = status === "All" || subscription.status === status;

      return matchesSearch && matchesStatus;
    });
  }, [subscriptions, searchTerm, status]);
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Subscriptions belonging to your organization.
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by phone number, employee or subscription number..."
          ...
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr className="border-b border-slate-200 text-left">
                ...
                <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Commitment Start</th>
                <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Commitment End</th>
            ...
            <tbody>
              {paginated.map((subscription) => (
                <tr key={subscription.id} className="border-b border-slate-100 transition hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4 text-sm text-slate-600">{formatDate(subscription.commitmentStartDate)}</td>
                  <td className="px-5 py-4 text-sm text-slate-600">{formatDate(subscription.commitmentEndDate)}</td>
                  <td className="px-5 py-4 text-right">
                    <Link
                      to={`/organization/subscriptions/${subscription.id}`}
        ...
        <div className="flex flex-col gap-3 border-t border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-slate-500">
            Showing {paginated.length} of {filtered.length} subscriptions
          </p>
